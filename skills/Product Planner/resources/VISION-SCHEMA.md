...
The `docs/vision.json` file captures all founder answers from the Product Planner intake conversation. It is the single input for document generation.
...
interface Vision {
  ...
  // Note: visual identity (mood, anti-patterns, tokens) is deliberately not
...
The `meta.version` field tracks the schema version — the shape of `docs/vision.json` (what fields exist, their types, valid enum values). The current version is `"1.0"`.

If you encounter a `docs/vision.json` whose shape doesn't match this document (extra fields, missing fields, or an older version number), reconcile it: migrate the data to the current shape, update `meta.version`, set `meta.updatedAt` to the current timestamp, and tell the user what changed. When the schema itself changes, bump the version and update this document and the example below.

## Validation

There is no validator script — validate by reading `docs/vision.json` against this document directly:

1. Every field in the type definition is present (except optional fields marked `?`).
2. No empty strings in required fields.
3. Enum fields (`platform`, `revenueModel`, `appType`, `codingAgent`) contain only the listed values.
4. The Field Rules below all hold.
